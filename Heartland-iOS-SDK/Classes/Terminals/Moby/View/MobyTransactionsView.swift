...
private struct MobyTransactionsView: View {
    ...
    public var body: some View {
                ...
                if self.showReceiptDialog, let image = self.image {
                    ...
                    ScrollView {
                        VStack {
                            ...
                            Button {
                                guard let device = RUUAHelper.sharedInstance.mobyDevice else {
                                ...
                                RUUAHelper.sharedInstance.mainTransaction = .credit
                            ...
                            Button {
                                ...
                                builder.allowDuplicates = NSNumber(value: self.allowDuplicates)
                                builder.isSurchargeEnabled = false
                                RUUAHelper.sharedInstance.mainTransaction = .auth
                                isTransactioning = true
                                builder.execute()
                            } label: {
                                Text("Credit Auth")
                            ...
                            Button {
                                ...
                                builder.transactionId = clientTransaction
                                builder.referenceNumber = self.terminalReferenceNumber
                                RUUAHelper.sharedInstance.mainTransaction = .capture
                                isTransactioning = true
                                builder.execute()
                            } label: {
                                Text("Credit Capture")
        ...
        .padding(16)
        .onReceive(RUUAHelper.sharedInstance.$transactionId) { value in
            guard let value = value else { return }
            self.clientTransaction = value
        }
        .onReceive(RUUAHelper.sharedInstance.$terminalRefNumber) { referenceNumber in
            guard let referenceNumber = referenceNumber else { return }
            self.terminalReferenceNumber = referenceNumber
        }
        .onReceive(RUUAHelper.sharedInstance.$showMessage, perform: { action in
            self.showMessage = action
        })
        .onReceive(RUUAHelper.sharedInstance.$isProcessing, perform: { isProcessing in
            self.isTransactioning = isProcessing
        })
        .onReceive(RUUAHelper.sharedInstance.$status, perform: { status in
            self.status = status
        })
        .alert(
            Text(RUUAHelper.sharedInstance.success ? "Success" : "Failure"),
            ...
            VStack {
                Text(RUUAHelper.sharedInstance.message)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
            
        }
        .onReceive(RUUAHelper.sharedInstance.$receiptImage, perform: { image in
            self.image = image
        })
    }
}
