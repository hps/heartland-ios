...
@interface HpsHeartSipTcpInterface () {
...
@implementation HpsHeartSipTcpInterface
...
-(void) connectionOpenBlock:(id<IHPSDeviceMessage>)message{
	...
	self.connectionOpenedBlock = ^(HpsHeartSipTcpInterface* connection){
...
-(void) messagesRecivedBlock{
	...
	self.messageReceivedBlock = ^(NSData* sipObject, NSString* error){
		...
		if (sipObject){
			...
			dispatch_async(dispatch_get_main_queue(), ^{
				...
				weakSelf.sendResponseBlock(sipObject, nil);
...
-(void)connectionsClosedBlock{
	...
	self.connectionClosedBlock = ^(HpsHeartSipTcpInterface* connection){
...
-(void)connectionsFailedBlock
{
	...
	weakSelf.connectionFailedBlock = ^(HpsHeartSipTcpInterface* connection){
		dispatch_async(dispatch_get_main_queue(), ^{
	...
	@try {
		if ([inputBuffer length] > 0 ){
			...
			if ([inputBuffer length] > 0) {
				...
				HpsHeartSipResponse *responseToCheckMultiMessage = [[HpsHeartSipResponse alloc]initWithXMLData:inputBuffer];
